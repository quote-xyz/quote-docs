...
- The endpoint pages in `api-reference/endpoints/` are **generated** by `scripts/gen-endpoint-pages.py` from the curated spec, which is itself **generated** from `quote-backend/docs/openapi.yaml` by `scripts/sync-openapi.py` (that spec is the source of truth). The sync writes the curated spec to **two** paths: `api-reference/openapi.yaml` and `.gitbook/assets/openapi.yaml`. The second is the one the pages load and the site serves — GitBook's UI created it and repointed every page at it — so writing only the first updates a file nothing reads. A guard in the sync fails if a page references a spec the script does not write. The sync script strips non-trading surfaces (Quentin/NL-order, Parallel news, daily quote). Never hand-edit either generated artifact or copy the backend spec over it verbatim; re-run the scripts instead
...
- Strategy names in code style: `passive_twap`, `vwap`, `iceberg`, `participation_rate`, `chase_limit`. The engine's `adaptive_is` strategy is deliberately undocumented; never add it to these docs. `scripts/sync-openapi.py` enforces this across every page, not just the generated reference
