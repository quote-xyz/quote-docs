...
This guide takes you from zero to a filled order over the API. You'll need a Quote account with a registered agent wallet — if you've traded in the [terminal](https://quotemarkets.xyz), you already have one.

<Steps>
  <Step title="Mint an API key">
    API keys are minted from a logged-in terminal session (they can't mint each other — see [Authentication](/authentication)). In the terminal, open **Settings → API Keys**, or call the endpoint directly with your Privy session token:
  ...
  <Step title="Sign a request">
    Every API-key request carries three headers: `X-Quote-Key`, `X-Quote-Timestamp` (milliseconds), and `X-Quote-Signature` — an HMAC-SHA256 over the canonical string `timestamp \n METHOD \n path_with_query \n body`.
  ...
  <Step title="Check your agent wallet">
    ...
    If `agentAddress` is missing or `expired: true`, complete agent setup in the terminal first (registration requires a signature from your main wallet, which API keys can't produce).
  </Step>
