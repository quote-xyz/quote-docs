...
Orders routed through Quote carry Quote's builder code, which is how Quote monetizes without spread markup. You approve the builder fee once during [agent setup](/concepts/agent-wallets); the fee is denominated in tenths of a basis point and attached server-side. All execution analytics report builder fees separately from exchange fees so you always see the full cost picture.
