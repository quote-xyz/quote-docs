...
API keys are minted from a logged-in terminal session; they cannot mint each other (see [Authentication](authentication.md)). In the terminal, open **Settings → API Keys**, or call the endpoint directly with your Privy session token:
...
{% hint style="info" %}
The timestamp must be within 30 seconds of server time (configurable server-side). Sign the _exact_ bytes you send: the path must include the query string, and the body must match byte-for-byte.
{% endhint %}
{% endstep %}
...
Orders are signed by your [agent wallet](concepts/agent-wallets.md). Verify one is registered:
...
Or hand the same size to the execution engine as a [TWAP](strategies/passive-twap.md) worked over 15 minutes:
...
{% hint style="warning" %}
A `200` response means the order was **accepted into the engine**, not that it filled. For algo strategies, `orderId` is the parent strategy ID. See [Order Lifecycle](concepts/order-lifecycle.md).
{% endhint %}
{% endstep %}
...
For live push updates, the terminal uses the [algo status WebSocket](websockets/algo-status.md).
{% endstep %}
{% endstepper %}

## Next steps

<table data-view="cards"><thead><tr><th></th><th></th><th data-hidden data-card-target data-type="content-ref"></th></tr></thead><tbody><tr><td><strong>Authentication in depth</strong></td><td>Scopes, the canonical signing string, and Privy-only endpoints.</td><td><a href="authentication.md">authentication.md</a></td></tr><tr><td><strong>Choose a strategy</strong></td><td>Which execution algorithm fits your order size and urgency.</td><td><a href="strategies/overview.md">overview.md</a></td></tr></tbody></table>
