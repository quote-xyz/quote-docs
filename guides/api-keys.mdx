...
<Note>
  Key management is **Privy-session only**: you mint, list, and revoke keys from a logged-in terminal session. An API key can never mint another API key — a leaked key can't escalate itself.
</Note>
...
- **What a leaked key can't do:** withdraw funds (the [agent wallet](/concepts/agent-wallets) can't withdraw), manage keys or invites (Privy-only), or act beyond its scopes. What it *can* do — trade within its scopes — is bad enough: revoke immediately.
