...
Triggers are server-side conditions that submit an order for you when they fire. Unlike venue-native trigger orders, Quote triggers can watch conditions Hyperliquid can't express — funding rates, open-interest changes, another wallet's trades, or the clock — and the resulting order can be anything the [order API](/guides/placing-orders) accepts, including a full [algo strategy](/guides/algo-orders).
